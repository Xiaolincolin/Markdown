...
### 权限
一、权限分为：所有者（user），群组（group），其他人（other）
- id 查看用户  
- chmod修改文件的权限![][png]  
二、修改命令：chmod <664> 文件名
  - 也可以用`u,g,o,a`来代替所有者，群组或者其他人设置权限，用`+`来增加权限，`-`来去掉权限`=`设置成想要的权限，权限有`r`只读`w`写，`x`可执行。
  -setuid `+s`是设置普通用户在不能权限不能执行可执行程序时加上`chmod u+s`后拥有超级用户的权限，相当于把使用者的权限提升到拥有者的权限。
  -setgid  `+s`主要是用于目录，主要用于共享目录，普通用户拥有创建者的权限，可以访问所有增加。
  -setick `+t`大多也是用于共享目录，就是设置settick后，使用者不能删除或者重命名文件，只要拥有者才能删除等。
  
三、权限修改：  
	- umask可以修改当前默认创造文件时的权限，chmod是修改某一个文件的权限，二umask是创建时不做说明时根据你上一个权限做修改后的权限  
	列如：当前权限是`--- rw- rw- rw-`，修改other的权限为空时是：`umask 0006`特别说明，要去掉other的rw则需要给rw在执行umask时将其变为1。对应的二进制是`000 000 000 110`没做修改就是0，修改other的rw权限则是1，相当于取反。  
	
	
四、超级用户`su`和`sudo`
- 直接`su`切换到root用户，完整`su -l [user]`
- `chown`是改变一个文件的拥有者，比如hello.txt的拥有者是xiaolin,我要改成root用户，则`chown [user] [filename]`，如果user是超级用户则`sudo chown [user] [filename]`也可以改组，`chgrp`，也可以两个一起改  
- `passwd [user]` 修改密码
...
- ps：显示当前进程
	- 可选项不加-，直接ps x,ps aux
- top:实时更新进程
	- 把top的进程放到后天运行：`top&`,调出后台运行的进程`jobs`,再敲`fg%1`,数字代码调哪一个进程，运行进程
	- 快捷键把当前运行的进程放到后天，`crtl+z`


六、环境变量
	- `printenv`打印当前的环境变量。
	- `set`打印环境变量和shell变量。
	- `vi .basrc`可以在后面加入自己配置的环境，列如加入别名永久生效，之后`source .bashrc`生效。

七、vi的使用
	- `/`是搜索功能，按n切换到下一个
	- 替换，`:%s/heloo/nihao/g`把所有的heloo替换成nihao,不加g就只替换一个
	- 同时编辑两个文件，vi 后面直接跟两个文件空格区分，n查看下一个文件,N查看上一个文件
